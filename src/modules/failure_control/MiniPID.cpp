...
#define EPS 1e6
#define IS_NONZERO(x) ((x) <= -EPS || (x) >= EPS)
...
void MiniPID::setI(double i){
	...
	// if(IS_NONZERO(maxIOutput)){
	if(maxIOutput <= -1e5 || maxIOutput >= 1e5){
...
void MiniPID::setOutputLimits(double minimum,double maximum){
	...
	if(IS_ZERO(maxIOutput) || maxIOutput>(maximum-minimum) ){
 ...
 */
void MiniPID::setDirection(bool _reversed){
 ...
 * @param setpoint
 */
void MiniPID::setSetpoint(double _setpoint){
...
*/
double MiniPID::getOutput(double actual, double _setpoint){
	double output;
	double Poutput;
	double Ioutput;
	double Doutput;
	...
	if(IS_NONZERO(minOutput - maxOutput) && !bounded(output, minOutput,maxOutput) ){
		errorSum=error;
	...
	else if(IS_NONZERO(outputRampRate) && !bounded(output, lastOutput-outputRampRate,lastOutput+outputRampRate) ){
		errorSum=error;
	}
	else if(IS_NONZERO(maxIOutput)){
...
void MiniPID::setOutputFilter(double strength){
	if(IS_ZERO(strength) || bounded(strength,0,1)){
